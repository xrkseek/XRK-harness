 ...
 */
import type { ClientContext, SessionId, SnapshotStore } from '@xrkseek/client-runtime/client'
...
import type {
  ...
  InputTriggerSource, PickAction, PickOutcome, SubmitEnvelope, TriggerChar, TriggerGuard,
} from '../types.ts'
...
export class InputTriggerController {
   ...
   * by source name. Empty while closed or when no source publishes a header.
   */
  ...
  /** Re-fetch the currently open menu without changing its hit or visible query. */
  ...
  private settle(
    ...
    // a header off a draft nobody descended into.
