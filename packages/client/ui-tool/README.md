...
Built-in tool cards share pure card models for `terminal`, `read`, `diff`, `search`, and `web` render intents. Unknown intent tags and malformed wire card data fall back to flattened Tool result text. The legacy `ToolDetails` / `conversation.details.tool` seat is demoted with the right Detail column; keep the component for direct tests only.
