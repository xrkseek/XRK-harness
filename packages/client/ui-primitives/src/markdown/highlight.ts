...
export class StreamingHighlightSession {
   ...
   * `undefined` means the caller renders its plain fallback.
   * @param code - the fence text accumulated so far (display-trimmed).
   * @param lang - the language hint (a markdown fence info string).
   * @returns one entry per line of `code`, or `undefined` for unknown or not-yet-loaded languages.
   */
