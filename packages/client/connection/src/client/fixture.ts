...
import type { CommandDescriptor, CommandExecution, CommandResult } from '@xrkseek/xrk-commands/types'
import { deriveEventMessage, foldSurface } from '@xrkseek/xrk-session/surface'
import type {
  ApiProxy, ClientRequest, ClientResponse, HistoryEntry, HostFrame, MuxFrame, RpcReceipt,
  ModelProviderGroup, ModelSelection, RpcRequest, RpcResponse, RpcResult, ServerRequest, ServerResponse, SessionSummary,
  ToolCallView, ToolEventView, ToolResultView, WorkspaceId, WorkspaceView,
} from './api.ts'
import type { RequestPayload, ResponseValue, RpcMethodMap } from '@xrkseek/xrk-host-apiproxy/api'
...
function createFixtureWorld(options: FixtureOptions): FixtureWorld {
  ...
  const fixturePresets = new Map<string, { trust: 'system' | 'user'; content: string }>([
    ['standard', { trust: 'system', content: "- id: tool-bash\n  name: '@xrkseek/xrk-tool-bash'\n" }],
    ['minimal', { trust: 'system', content: "- id: tool-web-search\n  name: '@xrkseek/xrk-tool-web-search'\n" }],
    ['my-agent', { trust: 'user', content: "- id: tool-read\n  name: '@xrkseek/xrk-tool-read'\n" }],
  ])
  ...
  const api: ApiProxy = {
    ...
    jobs: {
      kill: request => Promise.resolve(ok(request, { outcome: 'requested' as const })),
      background: request => Promise.resolve(ok(request, { accepted: true as const })),
    },
    host: {
      ...
      listOpenInApps: request => ok(request, { apps: ['explorer', 'cursor', 'vscode'] as const }),
      openInApp: request => ok(request, { opened: true as const }),
    },
    workspace: {
...
export class FixtureApiClient extends AbstractApiClient {
  ...
  private dispatch(
    ...
    switch (method) {
      ...
      case 'job.background': return this.api.jobs.background(request)
      case 'host.describe': return this.api.host.describe(request)
      case 'host.pickDirectory': return this.api.host.pickDirectory(request, new AbortController().signal)
      case 'host.listDirectory': return this.api.host.listDirectory(request, new AbortController().signal)
      case 'host.createDirectory': return this.api.host.createDirectory(request)
      case 'host.openPath': return this.api.host.openPath(request, new AbortController().signal)
      case 'host.listOpenInApps': return this.api.host.listOpenInApps(request)
      case 'host.openInApp': return this.api.host.openInApp(request, new AbortController().signal)
