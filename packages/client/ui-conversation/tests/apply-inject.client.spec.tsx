...
// draft carrying), the composer-bar stop face, and openDetails demotion
// (Detail panel unregistered). Complements
...
import type { SessionBehaviorOverrides } from '@xrkseek/client-test-runtime'
import { LocaleRuntime } from '@xrkseek/client-locale/client'
import type { ISession, SessionId } from '@xrkseek/client-runtime/client'
import { apply, inject } from '@xrkseek/client-ui-conversation/client'
import type {
...
describe('conversation slot inject API', () => {
  it('assembles the thin API side-effect-free', async () => {
    ...
    await chatView.injected.loadThrough(42)
    expect(b.sessionFake.loadThrough).toHaveBeenCalledWith(42)
  ...
  it('openDetails (chat view face) is a no-op after Detail-panel demotion', async () => {
    const b = await bench()
...
describe('details demotion', () => {
  it('conversation apply does not occupy the layout details slot', async () => {
    const b = await bench()
    expect(b.runtime.slots.entries('details')).toEqual([])
    await b.runtime.dispose()
  })
})
