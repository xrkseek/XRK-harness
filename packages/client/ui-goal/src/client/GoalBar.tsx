 ...
 * (inline form in the same strip), and clear. Goal creation lives on the `/goal`
 * command, not here: loading (undefined), no goal (null), and complete goals
 * render nothing. Durable state arrives as the projected whole snapshot;
 * process-local activation rides on the Face goal view (or an explicit prop).
 */

import { useCallback, useEffect, useRef, useState } from 'react'
import type { GoalActivation, GoalSnapshot } from '@xrkseek/xrk-goal/client'
import {
...
import type { PropsLocale, TranslateNS } from '@xrkseek/client-ui-slots'
...
export type GoalDockProps = import('@xrkseek/client-ui-slots').PropsRuntime<'conversation.input.dock'> & GoalBarActions & PropsLocale<'goal'>

/**
 * Face publishes activation on the goal view object. Read it without widening
 * the durable GoalSnapshot type (activation stays process-local by contract).
 */
function activationOf(goal: GoalSnapshot | null | undefined): GoalActivation | undefined {
  if (goal === undefined || goal === null) return undefined
  const value = (goal as GoalSnapshot & { activation?: GoalActivation }).activation
  return value === 'armed' || value === 'disarmed' ? value : undefined
}

/** Dock adapter: overlays Face activation on the durable goal projection. */
export function GoalDock({ useProjection, onEdit, onPause, onResume, onClear, t }: GoalDockProps) {
  ...
  const activation = activationOf(goal)
  return (
    <GoalBar
      goal={goal}
