...
The `/client` exports are the plugin body (`apply`/`inject`), `LayoutController`, the four owner-share interfaces, and the **layout insets contract** (`LayoutInsets` · `LAYOUT_INSET_CSS` · `LAYOUT_INSET_ATTR`): AppFrame publishes solved details / left-rail widths onto `document.documentElement` and `ctx.layout.insets` so any floating workbench plugin can yield with CSS variables instead of mutually excluding the overview. AppFrame, the panel store, and the concession solver remain package-internal.
