# @xrkseek/client-ui-deliverables
...
`ProducedFiles` renders the row between the closing message's body and its IconActions footer: a quiet label and one measured file lane. It shows the largest leading prefix that fits (up to six chips; basename text, full path as the `title`) while reserving the exact localized `+ N files` width, so the remainder stays visible without wrapping or horizontal scrolling. Chip click is **preview** via owner `openFile` (sidebar plugins may intercept `workspaces.openPath`); when `canOpenPath`, the overflow menu offers **default-app open** and **file-manager reveal**, calling Face `host.openPath` directly (including `reveal`) so an openPath wrap cannot swallow OS actions. Hidden-file **Show in folder** reveals the session cwd the same way. That OS surface appears only on loopback when the Host handshake reports `canOpenPath`. Design rationale: the [workspace file links Agent Note](../../../.agents/notes/implemented/feature/2026-07-31-web-workspace-file-links.md).
