...
`ProducedFiles` 在收尾消息正文与其 IconActions 之间渲染该行：一个低调的标签和一条经过测量的单行文件 lane。它展示能够放下的最大前缀（至多六个标签项；文本为文件名，完整路径作为 `title`），并为本地化后的精确 `+ N 个文件` 宽度预留空间，因此剩余计数始终可见，既不换行也不横向滚动。芯片点击走属主 `openFile` 做**预览**（侧栏插件可拦截 `workspaces.openPath`）；`canOpenPath` 时溢出菜单提供**默认应用打开**与**资源管理器定位**，直连 Face `host.openPath`（含 `reveal`），以免被 openPath wrap 吞掉。隐藏文件时第二行的**在文件夹中显示**对会话 cwd 同样走 reveal。该 OS 面只在 loopback 且 Host 握手报告 `canOpenPath` 时出现。设计原理：[workspace 文件链接 Agent Note](../../../.agents/notes/implemented/feature/2026-07-31-web-workspace-file-links.md)。
