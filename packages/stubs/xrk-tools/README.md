...
- **`timeoutMs` on a definition is declarative only** — the registry never enforces deadlines; enforcement requires the `@xrkseek/xrk-tool-call-timeout-policy` wrapper.
