 ...
 * execution pipeline.
 * @module @xrkseek/xrk-tools
 */

import { Context, Service } from '@xrkseek/cordis'
import z from '@xrkseek/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeOf, scopeTarget } from '@xrkseek/xrk-scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@xrkseek/xrk-scope'
import type { CallId, ContentBlock, ToolSchema } from '@xrkseek/xrk-llm'
import { assertNever, deepFreeze, HarnessError } from '@xrkseek/xrk-llm'
import type { Agent } from '@xrkseek/xrk-agent'
import { snapshotJsonValue } from '@xrkseek/xrk-session'
import type { JsonValue, UserMessage } from '@xrkseek/xrk-session'
import type { ToolProviderResult } from '@xrkseek/xrk-system-prompt'
import type { CodeRuntime } from '@xrkseek/xrk-code-runtime'
...
export type { JsonValue } from '@xrkseek/xrk-session'
...
// lives in its own UI-facing module; re-export it so `@xrkseek/xrk-tools`
...
declare module '@xrkseek/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@xrkseek/xrk-scope`): agent-scoped listeners receive only that agent's calls.
     ...
     * Scope-filtered dispatch (`@xrkseek/xrk-scope`): agent-scoped listeners receive only that agent's dispatches.
     ...
     * Scope-filtered dispatch (`@xrkseek/xrk-scope`): keyed by `exec.agent`.
...
export interface ToolDefinition extends ToolSchema {
   ...
   * Enforced by `@xrkseek/xrk-tool-call-timeout-policy` (a `tools/execute` wrapper); it
 ...
 * @internal
 */
export const TOOL_RUNTIME_SCHEDULER: unique symbol = Symbol('@xrkseek/xrk-tools.scheduler')
...
export class ToolRuntime extends Service {
  ...
  private requireCodeRuntime(mode: ToolPresentationMode): CodeRuntime {
    ...
    if (!runtime) {
      throw new Error(`dsh-tools: mode "${mode}" requires a code runtime — load a ctx.codeRuntime implementation (e.g. @xrkseek/xrk-code-runtime-worker-thread) or set tools mode to "native"`)
    }
