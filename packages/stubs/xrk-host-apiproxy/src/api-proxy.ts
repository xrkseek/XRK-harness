...
import { installModelSelection } from '@xrkseek/xrk-agent'
import type { Agent, ModelSelection, ModelSelectionRef, AgentOptions, AgentStatus } from '@xrkseek/xrk-agent'
import type {} from '@xrkseek/xrk-agent-presets/types'
import { AttachmentError } from '@xrkseek/xrk-attachment'
...
import type { SessionPersistence } from '@xrkseek/xrk-session-persistence'
import { SessionQueryError, type SessionSearchCursor } from '@xrkseek/xrk-session-query'
import { SubagentError } from '@xrkseek/xrk-subagent'
import type { SubagentListEntry as CatalogSubagentListEntry } from '@xrkseek/xrk-subagent'
import { isUserInvocable } from '@xrkseek/xrk-skill'
import type { Workspace, WorkspaceRecord } from '@xrkseek/xrk-workspace'
import {
  workspaceDomainState, workspaceRecord, WorkspaceId as brandWorkspaceId,
  WorkspaceMoveInvalidError, WorkspaceOrderInvalidError, WorkspaceUnknownSessionError,
} from '@xrkseek/xrk-workspace'
// Type-only: brings the `ctx.tools` Context merge into this program (viewFor reads presenters).
import {
  InvalidPresetIdError, PresetExistsError, PresetMountError,
  PresetNotWritableError, resolveSessionPreset, UnknownPresetError,
} from '@xrkseek/xrk-agent-presets'
import type { PresetBearingSession } from '@xrkseek/xrk-agent-presets'
import type {} from '@xrkseek/xrk-tools'
import type {
...
import type { SessionRawArtifact } from '@xrkseek/xrk-session-persistence'
import {
...
import type {} from '@xrkseek/xrk-session-projection'
// Type-only: resolves `ctx.get('tasks')` to the background job registry.
import type {} from '@xrkseek/xrk-jobs'
import type { JobSnapshot } from '@xrkseek/xrk-jobs'
// Type-only: resolves `ctx.get('sessionProjectionCache')` (the cold listing column).
import type {} from '@xrkseek/xrk-session-projection-cache'
// GoalError narrows domain rejections to their stable codes at the wire boundary.
import { GoalError } from '@xrkseek/xrk-goal'
import type { GoalRef as CoreGoalRef } from '@xrkseek/xrk-goal'
// Type-only edges: resolve the command-change stream and `ctx.get('skills')`.
import type {} from '@xrkseek/xrk-commands'
...
import type {} from '@xrkseek/xrk-cordis-host-runner/types'
import type {} from '@xrkseek/xrk-skill'
// The settings/credentials seams: brand guards run at this wire boundary; the
// service reads stay optional (`ctx.get`) so a composition without either
// provider still serves every other domain.
import { SettingsConflictError, settingsNamespace } from '@xrkseek/xrk-settings'
import type { SettingsDescriptor, SettingsNamespace, SettingsPathOp } from '@xrkseek/xrk-settings'
import { credentialRef } from '@xrkseek/xrk-credentials'
// Value edge: the rename impl narrows the title service's validation failure; the import also resolves `ctx.get('sessionTitle')`.
import { SessionTitleInvalidError } from '@xrkseek/xrk-session-title'
import type { CallId } from '@xrkseek/xrk-llm/brand'
import type { ScopeKey } from '@xrkseek/xrk-scope'
import type { ApprovalOutcome, ApprovalRequestId } from '@xrkseek/xrk-user-approval'
// Side-effect type import: resolves the `approval/request` waterfall and
// `ctx.get('approval')` without a value dependency on the seam (optional composition).
import type {} from '@xrkseek/xrk-user-approval'
...
} from '@xrkseek/xrk-user-questions'
import { UserQuestionError } from '@xrkseek/xrk-user-questions'
import { DirectoryPickerError } from '@xrkseek/xrk-host-directory-picker'
import {
...
function projectionsUnavailableError(): RpcError {
  return {
    code: 'internal',
    message: 'subagent catalog is unavailable: this deployment does not mount the sessionProjections registry (load @xrkseek/xrk-session-projection)',
    details: {},
  }
}
...
export function createApiProxy(ctx: Context, defaults: ApiProxyDefaults): ApiProxy {
  ...
  function goalServiceFor(agent: Agent): NonNullable<ReturnType<typeof ctx.get<'goals'>>> | { error: RpcError } {
    ...
    if (goals === undefined) {
      return { error: { code: 'internal', message: 'goal service is absent: neither this session\'s agent preset nor the host composition mounts @xrkseek/xrk-goal', details: {} } }
    }
    return goals
  }
  ...
  function settingsAbsent(): RpcError {
    return { code: 'internal', message: 'settings service is absent: this deployment does not mount a settings provider (e.g. @xrkseek/xrk-settings-file) in its composition', details: {} }
  }
  ...
  function credentialsAbsent(): RpcError {
    return { code: 'internal', message: 'credentials service is absent: this deployment does not mount a credential provider (e.g. @xrkseek/xrk-credentials-local) in its composition', details: {} }
  }
    ...
    sessions: {
      ...
      async search(request, signal) {
        ...
        if (sessionQuery === undefined) {
          return err(request, {
            code: 'internal',
            message: 'session search is unavailable: this deployment does not mount @xrkseek/xrk-session-query',
            details: {},
          })
        }
        try {
    ...
    skills: {
      ...
      async list(request) {
        ...
        if (skillRegistry === undefined) {
          return err(request, { code: 'internal', message: 'skill registry is absent: neither this session\'s agent preset nor the host composition mounts @xrkseek/xrk-skill', details: {} })
        }
