...
Public request, value, version, and failure types are exported from the package root and `@xrkseek/xrk-message-feedback/types`; [`src/types.ts`](src/types.ts) is their source.
...
```yaml
- id: message-feedback
  name: '@xrkseek/xrk-message-feedback'
  config:
    maxNoteBytes: 8192
```
