...
规范结果为 `{ todos, counts: { pending, inProgress, completed } }`；其 Native 渲染器返回精简的更新确认。工具还会写入完整 `todo/write` 会话事件。UI 订阅事件流，并自行渲染该持久化列表：[web 客户端](../../client/ui-conversation)基于站立计划（最近一次 `todo/write` 整表，跨回合保留直到下一次写入）显示计划条和专属工具行。

## 会话投影

当组合挂载了 `ctx.sessionProjections`（[`@xrkseek/xrk-session-projection`](../../session/session-projection/README.md)）时，本包在一个注入的子插件中注册 `todos` 投影单元：`init` = `null`（尚无写入）、`apply` = 从每个 `todo/write` 取整表并跨回合保留直到下一次写入（仅在首次写入前为 `null`；其余事件返回同一状态引用）、`view` = 恒等、`stateVersion` = 3。该键在本包中合并进 `SessionProjectionMap`（经 Service Definition 包的 `/types` 出口）；框架驱动该单元，载体通过历史尾页与 `session/projection` 推送帧提供该值。未挂载注册表的组合不受影响。产品 Face 经 `createTodosProjectionUnit` 注册同一套站立计划语义。
