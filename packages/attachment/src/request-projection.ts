/**
 * Pure request-projection geometry shared by model routes and soft-budget
 * pricing. Aligned with DSH `@deepseek-ai/dsh-attachment` request-projection.
 */
...
export function requestImageDimensions(
  ...
  if (width >= height) {
    ...
    while (
      projectedWidth * projectedHeight > maxPixels &&
      projectedWidth > 1
    ) {
      projectedWidth -= 1;
...
/**
 * Aspect-preserving dimensions with an exact long edge (short edge rounds).
 * A long edge at or above the source returns the source unchanged.
 */
