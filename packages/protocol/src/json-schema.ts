...
export const sessionEventJsonSchema = {
  ...
  oneOf: [
    ...
    {
      type: "object",
      required: ["type", "ts", "turnId", "reason", "header"],
      properties: baseProps({
        type: { const: "request/header" },
        turnId: { type: "string" },
        reason: { enum: ["initial", "resume", "change"] },
        header: { type: "object" },
      }),
      additionalProperties: false,
    },
    ...
    {
      type: "object",
      required: ["type", "ts", "turnId", "stepId", "retryId", "retry"],
      properties: baseProps({
        type: { const: "llm/retry-started" },
        turnId: { type: "string" },
        stepId: { type: "string" },
        retryId: { type: "string" },
        retry: { type: "integer", minimum: 1 },
