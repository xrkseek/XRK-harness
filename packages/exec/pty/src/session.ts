...
class LocalSendOperation implements TerminalSendOperation {
  ...
  private readonly promise = withResolvers<TerminalSendResult>();
  ...
  setInitialForeground(foreground: TerminalForeground | undefined): void {
...
export class LocalPtySession implements TerminalBackendSession {
  ...
  private readonly outputEnded = withResolvers<void>()
  ...
  constructor(
    ...
    private readonly config: PtyBackendConfig,
  ) {
  ...
  read(request: TerminalReadRequest): TerminalReadResult {
    ...
    const bounded = clipUtf8Tail(requested, this.config.maxReadBytes)
  ...
  private async onExit(outcome: TerminalOutcome): Promise<void> {
