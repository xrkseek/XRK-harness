...
the deliberate composition divergences from `xrk-harness serve` — are documented in
...
## Do not import `@xrkseek/client-*` here
...
`@xrkseek/xrk-goal/remote`, so the Host build phase ended up waiting on an
...
it imports `AppWebEntry` from `@xrkseek/client-web` and the boot-manifest
type from `@xrkseek/client-modules/client`: booting the real shell is what
...
`@xrkseek/client-runtime/client` because `client/runtime` is reachable
