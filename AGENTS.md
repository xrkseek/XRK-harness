...
- **外壳可复用、内核不可让**：聊天 UI 用已捕获的 DSH Web（`apps/web-static`），不自研平行壳；可在本仓裁剪 boot（省略 Cordis 空面）与品牌。session 事件真源 · TS 宿主 · 工具瀑布 · compose/presets 是本仓本质，Face 只对接 wire，不把 Cordis 嵌成 Host 内核
