...
import {
  clientSourceRoot,
  findUiI18nViolations,
} from "./verify-client-ui-i18n.ts";

function messages(source: string): string[] {
  return findUiI18nViolations(
    "packages/client/ui-example/src/client/View.tsx",
    source,
...
describe("Client UI i18n source check", () => {
  ...
  it("normalizes native separators before deriving a Client source root", () => {
    expect(
      clientSourceRoot("packages/extensions/sample/src/client/View.tsx"),
    ).toBe("packages/extensions/sample/src/client");
    expect(
      clientSourceRoot("packages\\extensions\\sample\\src\\client\\View.tsx"),
    ).toBe("packages/extensions/sample/src/client");
    expect(
      clientSourceRoot("packages/extensions/sample/src/server/index.ts"),
    ).toBeUndefined();
  });
  ...
  it("does not inspect locale dictionary owners", () => {
    ...
    expect(
      findUiI18nViolations(
        ...
        'export const en = { title: "Desktop menu" }',
      ),
    ).toEqual([]);
  });
  ...
  it("rejects Electron dialog, title, prompt, and DOM copy outside locale owners", () => {
    ...
    `;
    expect(
      findUiI18nViolations("apps/desktop/src/main.ts", source).map(
        (row) => row.text,
      ),
    ).toEqual([
      "Update available",
