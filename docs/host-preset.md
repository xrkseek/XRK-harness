...
| 平面 | 名字 | 职责 |
|------|------|------|
| Session | 五档徽章（Minimal … Harness） | tools + persona + pipeline + workspace inject；子代理 / 计划模式由 profile 决定 |
...
`server` 不是第三套工具表。产品徽章展示五档内置；遗留 wire 值 `server` → `harness`。选型表：[profiles.md](./profiles.md)。
...
| Plane | Name | Role |
|-------|------|------|
| Session | Five badges (Minimal … Harness) | tools + persona + pipeline + workspace inject; subagents / plan mode from profile |
...
`server` is not a third tool table. Product badges show five built-ins; legacy wire value `server` → `harness`. Selection table: [profiles.md](./profiles.md).
