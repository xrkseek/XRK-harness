...
| API | Meaning |
|-----|------|
| **newSession** | New log (or reuse an existing id) |
| **admit** | Bookkeeping only; **not** in `deriveMessages` (current default = queue) |
| **continueTurn(text)** | Run one turn on the session (`runTurn` writes `user/message`) |
| **continueTurn()** | `promoteAdmitsForTurn` (steer batch merge or one queue item) → then run |
| **run(text)** | Compatibility alias; text required |

`createAgent` already exposes `continueTurn` / `admit` / `pendingAdmits`.

## Events

```text
prompt/admitted  →  pending (not model-visible)
prompt/promoted  →  consumed admit (still not a chat message)
user/message     →  written by runTurn (model-visible)
```
...
|--------|------|------|
| `POST` | `/api/sessions` | newSession → `201 { sessionId }` |
| `POST` | `/api/sessions/:id/admit` | body `{ message, resume?, wake? }` — default **202** admit-only; `wake` → 202+scheduled; `resume` → drain join **200** |
| `POST` | `/api/sessions/:id/turn` | continueTurn; `message` may be omitted (promote) |
| `POST` | `/api/chat` | ensureSession + continueTurn(message) (convenience) |
