...
| `transport: "http"` → SDK `StreamableHTTPClientTransport` | 进程级自动重连 · Face MCP 设置 UI |
...
Host 批量接线见 [server-host.md](./server-host.md)（`XRK_MCP_*`；条目可 `command` 或 `url`）。
...
Face MCP 设置 UI · 进程级 supervisor（HTTP 仅 SDK SSE 恢复；`tools/list_changed` 热同步已接）。
