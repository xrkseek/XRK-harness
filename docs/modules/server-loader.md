...
| `types.ts` | `RegisteredPlugin` · `PluginPromptSection` | kind + 贡献字段；禁止环依赖回 prompt |
| `kinds.ts` | `PLUGIN_KINDS` · `RESERVED_*` · `isKnownPluginKind` | 新能力优先新 kind + apply* |
| `manifest.ts` | `xrk.plugin.json` / package 字段扫描 | 只发现不执行 |
| `load.ts` | 动态 import · 校验 export 形 | id/kind 与 manifest 一致 |
...
2. `tools` / `prompt` 判定一律用 `PLUGIN_KINDS.*`。  
3. 插件失败要可逆：`dispose` + unregister。  
4. MCP Host 接线产出的也是 `kind: "tools"` 插件（见 [server-host.md](./server-host.md)）。

## 测试

`packages/server/loader/tests/loader.test.ts` — discover/load · tools · prompt（含 base 优先）。
