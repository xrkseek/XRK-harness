...
node scripts/npm-prune-withdrawn.mjs            # 撤中间版（保留当前版 + 0.1.31）
```
...
| 档 | 版本 | 用途 |
|------|------|------|
| **当前（@latest）** | **0.2.2** | 0.2 预览线；`npm i -g @xrkseek/harness-cli@latest` |
| **上一预览** | **0.2.1** | 同线前一版 |
| **上一正式** | **0.1.31** | 稳定留档 |
...
GitHub Release 公开页只保留 **v0.1.31** 与 **v0.2.2**。
...
node scripts/npm-prune-withdrawn.mjs            # deprecate intermediate versions (keep current + 0.1.31)
```
...
| Line | Version | Use |
|------|---------|-----|
| **Current (@latest)** | **0.2.2** | 0.2 preview line; `npm i -g @xrkseek/harness-cli@latest` |
| **Previous preview** | **0.2.1** | Prior build on the same line |
...
The GitHub Releases page keeps only **v0.1.31** and **v0.2.2**.
