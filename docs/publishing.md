...
| 档 | 版本 | 用途 |
|------|------|------|
| **正式 · 当前（@latest）** | **0.3.1** | 奇数正式线；`npm i -g @xrkseek/harness-cli@latest` |
| **预览 · 末号** | **0.2.7** | 偶数预览线结束；对照留档，不推荐日常安装 |

昔日正式 **0.1.31** → 现 **0.3.1**（经 0.3.0）；昔日预览 **0.0.11**（已撤）→ 现末号 **0.2.7**。预览基线不再作为推荐入口；`preview` dist-tag 去掉。
...
GitHub Release 公开页只保留 **v0.3.1** 与 **v0.2.7**。
...
| Line | Version | Use |
|------|---------|-----|
| **Formal · current (@latest)** | **0.3.1** | Odd formal line; `npm i -g @xrkseek/harness-cli@latest` |
| **Preview · last** | **0.2.7** | Even preview line ended; archive only |

Formal **0.1.31** → **0.3.1** (via 0.3.0); preview **0.0.11** (withdrawn) → last **0.2.7**. Drop the `preview` dist-tag.
...
The GitHub Releases page keeps only **v0.3.1** and **v0.2.7**.
