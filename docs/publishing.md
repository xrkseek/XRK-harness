...
约定：`MAJOR.MINOR.PATCH` 的 **MINOR（第二位）**——**奇数正式**（`0.3.x`）、**偶数预览**（`0.2.x`）。**勿**用 PATCH 奇偶判档；正式线补丁顺序递增（`0.3.10` → `0.3.11`）。
...
昔日正式 **0.1.31** → 现 **0.3.11**（经 0.3.0/0.3.3/0.3.5/0.3.7/0.3.8/0.3.9/0.3.10）；昔日预览 **0.0.11**（已撤）→ 现末号 **0.2.7**。预览基线不再作为推荐入口；`preview` dist-tag 去掉。
...
GitHub Release 公开页只保留 **v0.3.11** 与 **v0.2.7**。
...
Rule: in `MAJOR.MINOR.PATCH`, **MINOR (second component) parity** — **odd = formal** (`0.3.x`), **even = preview** (`0.2.x`). **Not** PATCH parity; formal-line patches increment in order (`0.3.10` → `0.3.11`).
...
Formal **0.1.31** → **0.3.11** (via 0.3.0/0.3.3/0.3.5/0.3.7/0.3.8/0.3.9/0.3.10); preview **0.0.11** (withdrawn) → last **0.2.7**. Drop the `preview` dist-tag.
...
The GitHub Releases page keeps only **v0.3.11** and **v0.2.7**.
