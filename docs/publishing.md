...
| **预览 · 当前** | **0.4.0-rc.1** | `MINOR=4` 预览线首号；npm dist-tag `rc`，不覆盖 `@latest` |
| **预览 · 上一轮末号** | **0.2.7** | `MINOR=2` 预览线结束；对照留档，不推荐日常安装 |

昔日正式 **0.1.31** → 现 **0.3.11**（经 0.3.0/0.3.3/0.3.5/0.3.7/0.3.8/0.3.9/0.3.10）；预览 **0.0.11**（已撤）→ **0.2.7** → 现 **0.4.0-rc.1**（新预览线首号）。上一轮预览基线不再作为推荐入口。
...
预发布号（`-rc.N` / `-beta.N`）由 `release.mjs` 自动按版本首段 prerelease 标识附 `--tag`（`0.4.0-rc.1` → `rc`），并给 GitHub Release 加 `--prerelease`；因此 rc 既不进 npm `@latest`，也不占仓库 Latest 徽标。正式版无 `--tag`，行为不变。

GitHub Release 公开页保留 **v0.3.11**（正式当前）、**v0.4.0-rc.1**（预览当前）与 **v0.2.7**（上一轮对照）。
...
| **Preview · current** | **0.4.0-rc.1** | `MINOR=4` preview line head; npm dist-tag `rc`, does not move `@latest` |
| **Preview · previous line end** | **0.2.7** | `MINOR=2` preview line ended; archive only |

Formal **0.1.31** → **0.3.11** (via 0.3.0/0.3.3/0.3.5/0.3.7/0.3.8/0.3.9/0.3.10); preview **0.0.11** (withdrawn) → **0.2.7** → **0.4.0-rc.1** (new preview line head). The previous preview baseline is no longer a recommended entry point.
...
Prereleases (`-rc.N` / `-beta.N`) are published by `release.mjs` with an automatic `--tag` derived from the leading prerelease identifier (`0.4.0-rc.1` → `rc`), and the GitHub Release is created with `--prerelease`; an rc therefore never lands on npm `@latest` nor takes the repo's Latest badge. Plain versions get no `--tag`, so behaviour is unchanged.

The GitHub Releases page keeps **v0.3.11** (formal current), **v0.4.0-rc.1** (preview current) and **v0.2.7** (previous-line archive).
