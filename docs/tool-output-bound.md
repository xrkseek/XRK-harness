...
Generic cap on model-visible output. **Leaf tools may return full domain output**; the pipeline applies a uniform bound after `finalize`, then freezes `tool/result`.
...
- 不做执行授权（仍是 guards） / Does not authorize execution (still guards)
- 不替代叶内 capture 限额（如 shell stdout 自限） / Does not replace leaf capture limits
- 会话日志存 **bound 后** 文本（模型可见 ≡ 日志） / Session log stores **post-bound** text
- 完整正文在 workspace 相对路径（默认 `.xrk/tool-outputs/…`），不进 session 事件 / Full body stays on a workspace-relative path and does not enter session events
