...
三态：**能跑 / 未稳 / 未做**。与代码对齐。基线 **v0.1.20**（当前正式公开线）。**本页主路径表当前均为能跑**；未稳/未做仅用于后续新增能力登记。
...
| 层级 | 能做什么 | 前置 |
| --- | --- | --- |
| **A — 能用** | `npm i -g @xrkseek/harness-cli` 后 `xrkh web`/`run`，或源码 `build` + 组装壳后跑；**v0.1.20** 当前正式公开发版 | Node ≥26；真模型需 brand `apiKeyEnv` 或 replay |
...
Three states: **Working / Unstable / Not done**. Aligned with code. Baseline **v0.1.20** (current formal public line). **Main-path rows on this page are Working today**; Unstable/Not done are reserved for newly tracked gaps.
...
| --- | --- | --- |
| **A — Usable** | `npm i -g @xrkseek/harness-cli` then `xrkh web`/`run`, or source `build` + assembled shell; **v0.1.20** is the current formal public release | Node ≥26; live models need brand `apiKeyEnv` or replay |
