...
三态：**能跑 / 未稳 / 未做**（Working / Unstable / Not done）。与代码对齐。基线 **v0.1.14**（当前正式公开线）。

Three states: **Working / Unstable / Not done**. Aligned with code. Baseline **v0.1.14** (current formal public line).
...
| --- | --- | --- |
| **A — 能用** | `npx @xrkseek/harness-cli` 或源码 `build` + 组装壳后 `web`/`run`；**v0.1.14** 当前正式公开发版 | Node ≥26；真模型需 brand `apiKeyEnv` 或 replay |
