...
| HTTP + Host + Face 主路径 | `server-*`（产品 boot 省略 Cordis 客户端与 HMR；Face Cordis RPC 仍空 inventory；`GET/HEAD /api/session.export`；`host.pickDirectory` 系统选目录；工具卡 presenter lookup + `session/jobs`；standing 工具表冷 history；host 流 session-added 带子会话字段 / workspace-removed / order-changed；`question/requested` + `ask_user`） | [http-api.md](./http-api.md) · [host-face.md](./host-face.md) |
