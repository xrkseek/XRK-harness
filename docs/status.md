...
三态：**能跑 / 未稳 / 未做**。与代码对齐。基线 **v0.1.24**（当前正式公开线）。**本页主路径表当前均为能跑**；未稳/未做仅用于后续新增能力登记。
...
| MCP | `@xrkseek/mcp`（stdio/HTTP 有界进程重连 + SSE；有序 content 投影；可选 image → AttachmentStore）；Host `XRK_MCP_*` 或 Face `mcp.servers` + `allowConnect` 落盘热挂载（policy deny → **park**）；产品挂载剧本 **`xrk-capability-attach`**（CLI 种子 → `~/.xrk/skills`） | [modules/mcp.md](./modules/mcp.md) · [host-face.md](./host-face.md) · [skills-layers.md](./skills-layers.md) |
...
| 层级 | 能做什么 | 前置 |
| --- | --- | --- |
| **A — 能用** | `npm i -g @xrkseek/harness-cli` 后 `xrkh web`/`run`，或源码 `build` + 组装壳后跑；**v0.1.24** 当前正式公开发版 | Node ≥26；真模型需 brand `apiKeyEnv` 或 replay |
...
Three states: **Working / Unstable / Not done**. Aligned with code. Baseline **v0.1.24** (current formal public line). **Main-path rows on this page are Working today**; Unstable/Not done are reserved for newly tracked gaps.
...
| MCP | `@xrkseek/mcp` (stdio/HTTP bounded process reconnect + SSE; ordered content projection; optional image → AttachmentStore); Host `XRK_MCP_*` or Face `mcp.servers` + `allowConnect` file-backed hot-mount (policy deny → **park**); attach playbook **`xrk-capability-attach`** (CLI seeds → `~/.xrk/skills`) | [modules/mcp.md](./modules/mcp.md) · [host-face.md](./host-face.md) · [skills-layers.md](./skills-layers.md) |
...
| --- | --- | --- |
| **A — Usable** | `npm i -g @xrkseek/harness-cli` then `xrkh web`/`run`, or source `build` + assembled shell; **v0.1.24** is the current formal public release | Node ≥26; live models need brand `apiKeyEnv` or replay |
