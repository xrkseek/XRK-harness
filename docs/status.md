...
三态：**能跑 / 未稳 / 未做**。与代码对齐。基线 **v0.4.4**（`0.4.x` 补丁线，接管 `@latest`；上一补丁 **v0.4.3**；上一正式线 **v0.3.11**；上一轮预览末号 **v0.2.7**；旧正式文稿 **v0.1.31**）。**本页主路径表当前均为能跑**；扩展能力节另列已补 / 暂缓（标 **未做** 不得当作已支持）。
...
| 任意类型文件上传 · 侧栏预览 · 可继续子代理排队/Steer/Stop · Open in · feedback | **能跑**（见主路径表与 [v0.4.4](./releases/v0.4.4.md)；`pnpm test:web` **21/21**） |
...
| Desktop 整包 | **未稳**（ADR-0008 Accepted：第一波 `package:desktop` 流水线已开；`XRK_DESKTOP_PACKAGE=1` 产包需 optional electron-builder；公开签名发版/更新频道上传仍待凭据） |
...
| 学习环（任务后提案 skill） | `propose_skill`：草案 → Face 人确认 → `.agents/skills`；复杂回合 fragment nudge；**skill curator** 确定性归档过期工作区 skill（`.archive/`，无 aux LLM / background fork） | **能跑**（见 [skills-layers.md](./skills-layers.md)） |
...
| Desktop 产品入口 | 默认入口仍是 `xrkh web` / CLI。阶段 **first-wave-package**：`pnpm package:desktop` 校验/产包；`pnpm dev:desktop` 仍为开发投影。`isDesktopProductReady()===true`（流水线就绪）；`installerShipped===false` | **未稳**（ADR-0008；勿把安装包当 day-1 入口） |
| Agent Teams / 协作图 | 子代理委托之上的图 + **任务板**：`delegates`/`peer` 边；spawn 角色模板；`output_schema`；queue|steer；takeover；**managed worktree** 分配/回收/owner 元数据并绑定 `teamTasks.worktree*`；**`mergeIntoParent`（ff-only 合回主仓）**；Status Overview **任务板 UI**（worktree · resultPreview · 打开子会话 · 暂停接管 · 恢复） | **能跑** |
| WorkflowEngine · Ralph | Cordis 抽象 `WorkflowEngine` + **`InProcessWorkflowEngine`**（AsyncFunction）+ **`IsolatingWorkflowEngine`**（worker_threads 脚本体隔离；`agent()` 在 worker 内恒为 null，非完整 DSH PTC SDK）；产品 Face 模型面 **`ralph`** 新鲜子代理环；Cordis `workflow` 工具仍依赖 Host 挂载引擎 | Face `ralph` **能跑**；InProcess / Isolating Provider **能跑**（需挂载）；完整 PTC SDK **未做** |
...
| 压缩分阶可观测 | Soft-budget / overflow **prune→summary** 阶段折进 Status/`/status`；**策略族** `compactionStrategy`：`prune-summary` · `prune-only` · `summary-only` · `off`；`delivery` 暴露 turn ↔ compact 互斥 | **能跑** |
| Spill locator · 概况打开 | `@xrkseek/spill`：locator / store / policy；Status 列出近期 `spillPaths` +「打开 spill」→ `host.openPath`；`contextTimeline` 带 `spillPath` | **能跑**（见 [tool-output-bound](./tool-output-bound.md)） |
| 成本 / 账单视图 | Status/`/status` 投影会话 **`costUsage`**（totals · `byModel` · `byProviderModel`）+ Host **cost-meter ledger** 跨会话今日/本月/累计与按模型榜 + **`billing.dailyTrend` 近 14 日柱图**；`session.export` ZIP 含 `cost.json`；社区 `costMeter/*` · tokenledger 仍为同源 ledger | **能跑** |
| 舰队健康 / 通道告警 | Status `fleet`（health · slots · inbox · alerts）+ `channels.alerts`（bridge/stub IM）；`/status` 文本同源 | **能跑** |
| 上下文浏览器 | Overview「上下文」页签：独立浏览 inject / compact / spill；Status 内仍有精简时间线 | **能跑** |
...
| 策展记忆 | `memory`：`{XRK_HOME}/memories` 冻进系统提示；工具 add/replace/remove；回合后启发式笔记；会话结束 **Phase1** + 可选 **Phase2 LLM**（`phase2Llm` / `XRK_CURATED_MEMORY_PHASE2`）。与 Mnemon 分开 | **能跑**（见 [curated-memory](./curated-memory.md)；Settings → Plugins → 策展记忆） |
...
| MemoryProvider（可插拔策展后端） | `@xrkseek/exec-memory`：`file` 默认 · `http` · **`sqlite`**（`node:sqlite`）；`resolveMemoryProvider` / `XRK_MEMORY_PROVIDER` | **能跑**（见 [curated-memory](./curated-memory.md)） |
...
| A2A 出站平台插件 | `@xrkseek/a2a` + `extensions/a2a`：`a2a_discover/call/list/history`；`context_id` JSONL 持久 · 环路上限；**入站**（`XRK_A2A_INBOUND=1`）：Agent Card + `POST /a2a` `message/send` stub（无 SSE/push/Face 注入） | **能跑**（可选装 / 入站 opt-in） |
...
| apply_patch · managed worktree 合回 | Codex 格式 `apply_patch`（失败卡带 `meta.code`）；子代理 `worktree:true`；`mergeIntoParent` **ff-only**（冲突 retain）；Face `worktree.merge`；Status / Overview 展示 `worktreeLeaseStatus`（retained 文案） | **能跑** |
...
| 层级 | 能做什么 | 前置 |
| --- | --- | --- |
| **A — 能用** | `npm i -g @xrkseek/harness-cli` 后 `xrkh web`/`run`，或源码 `build` + 组装壳后跑；**v0.4.4** 当前 `@latest`（上一补丁 **v0.4.3**；上一正式线 **v0.3.11**；上一轮预览末号 **v0.2.7**） | Node ≥26；真模型需 brand `apiKeyEnv` 或 replay |
...
Three states: **Working / Unstable / Not done**. Aligned with code. Baseline **v0.4.4** (`0.4.x` patch line, owning `@latest`; previous patch **v0.4.3**; previous formal line **v0.3.11**; previous preview line end **v0.2.7**; legacy formal notes **v0.1.31**). **Main-path rows on this page are Working today**; the extended-capabilities section lists filled / deferred items (**Not done** must not be treated as supported).
...
| Arbitrary file upload · sidebar preview · continuable subagent queue/Steer/Stop · Open in · feedback | **Working** (see main-path table and [v0.4.4](./releases/v0.4.4.md); `pnpm test:web` **21/21**) |
...
| Desktop full package | **Unstable** (ADR-0008 Accepted: first-wave `package:desktop` pipeline open; `XRK_DESKTOP_PACKAGE=1` needs optional electron-builder; public signed ship / update upload still credential-gated) |
...
| Learning loop (propose skill after tasks) | `propose_skill`: draft → Face confirm → `.agents/skills`; fragment nudge; **skill curator** archives stale workspace skills to `.archive/` (deterministic; no aux LLM / background fork) | **Working** (see [skills-layers.md](./skills-layers.md)) |
...
| Desktop product entry | Default entry remains `xrkh web` / CLI. Phase **first-wave-package**: `pnpm package:desktop` validates/produces; `pnpm dev:desktop` remains the development projection. `isDesktopProductReady()===true` (pipeline ready); `installerShipped===false` | **Unstable** (ADR-0008; installer is not the day-1 entry) |
| Agent Teams / collab graph | Graph on top of subagent delegation **plus task board**: `delegates`/`peer` edges; spawn roles; `output_schema`; queue|steer; takeover; **managed worktree** allocate/reclaim/owner metadata bound to `teamTasks.worktree*`; **`mergeIntoParent` (ff-only fold-back)**; Status Overview **task-board UI** (worktree · resultPreview · open child · pause/takeover · resume) | **Working** |
| WorkflowEngine · Ralph | Cordis abstract `WorkflowEngine` + **`InProcessWorkflowEngine`** (AsyncFunction) + **`IsolatingWorkflowEngine`** (worker_threads script-body isolation; `agent()` null in worker — not full DSH PTC SDK); product Face model surface **`ralph`** fresh-child loop; Cordis `workflow` tool still needs a mounted engine | Face `ralph` **Working**; InProcess / Isolating Provider **Working** (when mounted); full PTC SDK **Not done** |
...
| Compaction stage observability | Soft-budget / overflow **prune→summary** stages fold into Status/`/status`; **strategy family** `compactionStrategy`: `prune-summary` · `prune-only` · `summary-only` · `off`; `delivery` exposes turn ↔ compact exclusion | **Working** |
| Spill locator · overview open | `@xrkseek/spill`: locator / store / policy; Status lists recent `spillPaths` + “Open spill” → `host.openPath`; `contextTimeline` carries `spillPath` | **Working** (see [tool-output-bound](./tool-output-bound.md)) |
| Cost / billing view | Status/`/status` projects session **`costUsage`** (totals · `byModel` · `byProviderModel`) plus Host **cost-meter ledger** cross-session today/month/total, ranked models, and **`billing.dailyTrend` 14-day bars**; `session.export` ZIP includes `cost.json`; community `costMeter/*` · tokenledger stay on the same ledger | **Working** |
| Fleet health / channel alerts | Status `fleet` (health · slots · inbox · alerts) + `channels.alerts` (bridge/stub IM); same facts in `/status` text | **Working** |
| Context browser | Overview **Context** tab browses inject / compact / spill independently; Status still keeps a compact timeline | **Working** |
...
| Curated memory | `memory`: frozen `MEMORY.md` / `USER.md`; tool add/replace/remove; after-turn heuristics; session-end **Phase1** + optional **Phase2 LLM** (`phase2Llm` / `XRK_CURATED_MEMORY_PHASE2`). Separate from Mnemon | **Working** (see [curated-memory](./curated-memory.md); Settings → Plugins → Curated memory) |
...
| MemoryProvider (pluggable curated backend) | `@xrkseek/exec-memory`: `file` default · `http` · **`sqlite`** (`node:sqlite`); `resolveMemoryProvider` / `XRK_MEMORY_PROVIDER` | **Working** (see [curated-memory](./curated-memory.md)) |
...
| A2A outbound platform plugin | `@xrkseek/a2a` + `extensions/a2a`: `a2a_discover/call/list/history`; `context_id` JSONL persistence · anti-loop; **inbound** (`XRK_A2A_INBOUND=1`): Agent Card + `POST /a2a` `message/send` stub (no SSE/push/Face inject) | **Working** (optional / inbound opt-in) |
...
| apply_patch · managed worktree fold-back | Codex-format `apply_patch` (failure card with `meta.code`); subagent `worktree:true`; `mergeIntoParent` **ff-only** (retain on conflict); Face `worktree.merge`; Status / Overview shows `worktreeLeaseStatus` (retained copy) | **Working** |
...
| --- | --- | --- |
| **A — Usable** | `npm i -g @xrkseek/harness-cli` then `xrkh web`/`run`, or source `build` + assembled shell; **v0.4.4** is the current `@latest` (previous patch **v0.4.3**; previous formal line **v0.3.11**; previous preview line end **v0.2.7**) | Node ≥26; live models need brand `apiKeyEnv` or replay |
