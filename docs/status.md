...
| Exec / Workspace / Policy | `exec-*`（含 `exec-web`：`web_search` / `web_fetch`；**`exec-lsp`：`lsp`**；**`exec-pty`：`terminal_*` 六件套**）· `workspace`（inject · recipes · **`skill` 工具** · `/skill-name` 入账）· `policy` | [seams.md](./seams.md) · [web-tools.md](./web-tools.md) · [lsp-tools.md](./lsp-tools.md) · [pty-tools.md](./pty-tools.md) · [workspace-inject.md](./workspace-inject.md) · [slash-recipes.md](./slash-recipes.md) · [policy.md](./policy.md) |
| HTTP + Host + Face 主路径 | `server-*`（产品 boot 省略 Cordis 客户端与 HMR；Face Cordis RPC 仍空 inventory；`GET/HEAD /api/session.export`；`host.pickDirectory` 系统选目录；工具卡 presenter lookup + `session/jobs`；standing 工具表冷 history；host 流 session-added 带子会话字段 / workspace-removed / order-changed / `host/remote-event`；`question/requested` + `ask_user`；`todos` · `permissions` · `plan` 投影；`/permission` · `/plan` 真改 knobs；`/compact` 写 `context/compaction`；`/export` 请求壳下载；`/feedback` 写 `feedback/record`；`settings.describe` mcp 只读 inventory） | [http-api.md](./http-api.md) · [host-face.md](./host-face.md) |
