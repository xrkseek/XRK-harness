...
三态：**能跑 / 未稳 / 未做**。与代码对齐。基线 **v0.3.0**（当前正式线；上一正式 **v0.1.31**；0.2 预览末号 **v0.2.7**）。**本页主路径表当前均为能跑**；未稳/未做仅用于后续新增能力登记。
...
| 层级 | 能做什么 | 前置 |
| --- | --- | --- |
| **A — 能用** | `npm i -g @xrkseek/harness-cli` 后 `xrkh web`/`run`，或源码 `build` + 组装壳后跑；**v0.3.0** 当前正式线（上一正式 **v0.1.31**） | Node ≥26；真模型需 brand `apiKeyEnv` 或 replay |
...
Three states: **Working / Unstable / Not done**. Aligned with code. Baseline **v0.3.0** (current formal line; previous formal **v0.1.31**; last 0.2 preview **v0.2.7**). **Main-path rows on this page are Working today**; Unstable/Not done are reserved for newly tracked gaps.
...
| --- | --- | --- |
| **A — Usable** | `npm i -g @xrkseek/harness-cli` then `xrkh web`/`run`, or source `build` + assembled shell; **v0.3.0** is the current formal release (previous formal **v0.1.31**) | Node ≥26; live models need brand `apiKeyEnv` or replay |
