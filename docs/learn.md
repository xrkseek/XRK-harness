...
- **后台任务**：有 jobs 源才发 `session/jobs`；订阅基线只在集合非空时发（空集靠缺帧）；变更推全量快照（清空仍发 `[]`）。无主任务扇出到每个已列出会话。`JobView` 不含 `ownerSession` / `reported` / `outputLimitBytes`。Jobs 内核：`running`→`stopping`→终态、`bash-N` / `<kind>-N`、并发上限 10、`dispose` teardown 置 `reported`
