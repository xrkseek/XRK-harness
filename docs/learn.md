...
- **工具卡**：工具声明 `presentCall` / `presentResult`；Face mux/history `viewFor` 只 lookup（抛错 / 缺 pairing / 无 presenter → 没 view，壳走 generic）。`bash` 模型文本用 DSH `[exit code: N]`。不写 session 日志；history 用同一次扫描的 call pairing
