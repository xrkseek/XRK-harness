...
1. Unary：`POST /api/<method>`（点号如 `session.prompt`，或 Typert `commands/execute`）或 `/api/face/<method>` / Unary: `POST /api/<method>` (dot form such as `session.prompt`, or Typert `commands/execute`) or `/api/face/<method>`
2. WS：`/api/events.mux` · `/api/events.host`（及 `/api/face/events.*`）
3. 认证：`XRK_API_KEY`（Bearer / `x-api-key`）；产品壳同源回环可无头 / Auth: `XRK_API_KEY` (Bearer / `x-api-key`); product-shell same-origin loopback may omit headers
4. 未实现 → `ok: false` + 稳定 `error.code`（禁止假成功） / Unimplemented → `ok: false` + stable `error.code` (no fake success)

## 信封 / Envelope

| 方向 / Direction | 形状 / Shape |
|------|------|
| Client → Host | `{ type?: "client-request", rpcId, payload }` |
| Host → Client（unary） | `{ type: "server-response", rpcId, result }` |
| Host → Client（WS） | `{ rpcId, payload }` mux / host 帧 / mux / host frames |

## RPC 状态 / RPC Status

| 状态 / Status | 方法 / Methods |
|------|------|
| 能跑 / Runnable | `host.describe` · `host.pickDirectory` · `host.listDirectory` · `host.createDirectory` · `host.openPath` · `session.create/list/history/search/prompt/cancel/models/selectModel/rename/updateQueue/fork/respondApproval` · `session.attachment`（须附件仓 + 事件引用 / requires attachment store + event refs） · `GET/HEAD /api/session.export`（ZIP：JSONL + 子会话 + 附件；HEAD 先探活 / ZIP: JSONL + child sessions + attachments; HEAD probes first） · `agentPreset.list/select/read` · `workspace.list/create/rename/archiveSession/delete/insertBefore/insertSessionBefore` · `workspace.describe/listProduct/previewInject/syncSeeds` · `settings.get/set/describe/mutate/update/replace/openDocument` · `credentials.list/describe/set/unset` · `llm.providers/models/discoverModels` · `skill.list` · `commands/list` · `commands/execute`（内建 `/compact` · `/export` · `/feedback` · `/goal` · `/permission` · `/plan` · 进程 `kind: commands` 优先，否则 workspace recipe → `command/run`+`command/done` / built-ins; process `kind: commands` first, else workspace recipe → `command/run`+`command/done`） · `pluginInventory/list`（进程插件 + 产品壳 `boot.json` 条目；`kind: cordis` 为 `enabled: false` / `fiberPhase: failed` / process plugins + product-shell `boot.json` entries; `kind: cordis` → `enabled: false` / `fiberPhase: failed`） · `fileReferences/list` · `sessionReferenceResolver/candidates`（产品壳 `@file` / `@session` 补全；会话候选仅 id/cwd/title 元数据，不含跨会话 prepare / product-shell `@file` / `@session` completion; session candidates are id/cwd/title metadata only, no cross-session prepare） · `subagent.list/history/prompt/interrupt`（`session.create({ parentSessionId })` 或 `session.fork` 登记子会话；无 ACP 外挂 / child sessions via `session.create({ parentSessionId })` or `session.fork`; no ACP sidecar） · `messageFeedback/list/put/delete`（进程内 CAS；`messageId` = `{turnId}:{stepId}` / in-process CAS） · `goals/create|edit|pause|resume|complete|clear` 与 DSH 点号别名 `goal.*`（投影 `goal`；`turn/end` 在 armed 时续 admit，达 `maxGoalRounds` 则 `blocked`；Host 旁路 `{XRK_SESSIONS_DIR}/goals.json` / projection `goal`; armed `turn/end` continues admit; `blocked` at `maxGoalRounds`; Host sidecar `{XRK_SESSIONS_DIR}/goals.json`） · `dynamicCordisRunner/*` 空 inventory / no-op（不 `apply` Cordis / empty inventory / no-op; no Cordis `apply`） |
| 诚实拒绝 / Honest reject | `agentPreset.copy/remove/openDocument` → `agent-preset-read-only`（`authorable: false`；壳侧按钮已禁用 / shell buttons disabled） |

### Wire 与流行为 / Wire and Stream Behavior
...
- 静态资源 / Static assets：`XRK_WEB_DIST` → CLI `product-web/` → monorepo `apps/web/dist`
- CLI monorepo 缺默认 dist 时代跑 `web:build` + `client:bundle` + `web:assemble` / When monorepo default dist is missing, CLI runs `web:build` + `client:bundle` + `web:assemble`
- `apps/web`：产品壳源码；发行时拷进 `@xrkseek/harness-cli` 的 `product-web/` / Product-shell source; shipped into `@xrkseek/harness-cli` `product-web/`

## 相关 / Related

[http-api.md](./http-api.md) · [status.md](./status.md) · **[modules/server-face.md](./modules/server-face.md)**（文件地图 / file map）· [modules/server-host.md](./modules/server-host.md)
