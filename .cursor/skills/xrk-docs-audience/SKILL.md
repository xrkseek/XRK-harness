...
2. 中文半部文首：`> **读者**：…`；英文半部文首：`> **Audience**: …`（勿夹「请去看某某」旁白）。  
...
7. **语气**：教科书，不是日记。禁止自证/对照辩解（「未抄某某」「不是 Fork」「非上游营销体」「对齐某某以自证」）。边界用中性落点事实即可。教科书陈述本仓契约；**不要**写第三方产品对照表。可一句点到「吸收 Codex 与业界 harness 长处」，细节落本仓 API。社区兼容器包 id（如 `dsh-compat`、路径 `/_dsh/`）可作技术标识，正文用「社区兼容器 / community compat」。
